...
public class InMemoryLogMessageDAO implements LogMessageDAO {
    ...
    public void deleteMessage(int logMessageId) {
        if (!idToMessage.containsKey(logMessageId)) {
            throw new DAOException("Cannot delete message with id " + logMessageId + " because no record with this id exists.");
        }

        idToMessage.remove(logMessageId);
    }
